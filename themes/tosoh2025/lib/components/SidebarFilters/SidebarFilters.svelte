<script lang="ts">
  ...
  import type { FilterState } from '../../factories/createFilterState.svelte';
...
<div
  class={`bg-ghost-white p-md h-fit rounded-lg transition-all duration-100 lg:sticky lg:top-[8rem] lg:z-10 lg:min-w-[16rem] xl:min-w-[20rem] ${filterState.isLoading ? 'animate-pulse' : ''}`}
>
  ...
  <SearchInput
    ...
    disabled={isParentLoading || filterState.isLoading || filterState.hasError}
    {isActivatedQuery}
  />
  ...
  >
    {#each filterState.filtersArray as columnId}
      {@const filter = getFilter(
        filterState.topicFilters,
        columnId as string
      ) as TopicFilters['filters'][number]}

      {#if !filterState.searchColumnIds?.includes(columnId as string)}
        <TopicFilter
          {filter}
          options={(filterState.allOptions as FilterWithOptions)[columnId as ColumnId] || []}
          name={columnId}
          disabled={isParentLoading || filterState.isLoading || filterState.hasError}
